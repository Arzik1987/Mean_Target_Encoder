...
When dealing with non-numeric features a user of such model have two options &mdash; either drop these features, potentially losing important information they may contain, or map (*encode*) them into numeric space.
...
1. How exactly does MTE cause overfitting &mdash; does it affect all categorical features equally or some of them are more prone to overfitting than the other?
...
How to define which splits to consider? For all feature types that can be ordered, the *order* of observed values restricts a variety of possible splits. 
...
In this case, MTE is a trick to speed up the decision tree learning algorithm that *does not affect the result* by its own.
